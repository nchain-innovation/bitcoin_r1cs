...
#[test]
fn test_universal_pay_to_utxo_groth16() {
    ...
    // Dummy RefTx circuit for key generation
    let reftx_for_setup = RefTxCircuit::<TestPayToUTXO, ScalarFieldMNT4, Config> {
    ...
    let (pk, vk) = Groth16::<MNT4_753>::circuit_specific_setup(reftx_for_setup, &mut rng).unwrap();
    ...
    let input = reftx_pay2utxo.public_input();
    let proof = Groth16::<MNT4_753>::prove(&pk, reftx_pay2utxo, &mut rng).unwrap();
    let is_valid = Groth16::<MNT4_753>::verify(&vk, input.as_slice(), &proof).unwrap();
    assert!(is_valid);
